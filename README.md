...
```bash
bun add @principia/restate
```

## Usage

```js
import { createRestateAdmin, withValidation, cronJob, cronJobInitiator } from '@principia/restate'
...
```js
import { checkServiceHandler } from '@principia/restate/lib/Admin.js'
```
...
| [lib/Validation.js](./lib/Validation.js) | `withValidation` — wraps a workflow handler with input/output JSON schema validation via `@principia/common`.                                                                                                 |
