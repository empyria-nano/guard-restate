...
/**
 * Typed wrapper around `@restatedev/pubsub`'s `createPubsubObject`; see {@link defineWorkflow}.
 * Unlike `defineWorkflow`/`defineService`/`defineObject`, there's no handler map to write —
 ...
 * naming consistency, not to add behavior. Register the result alongside your other
 * services/objects on the same endpoint (see `setupRestate`'s `services` param) — that's what
 * makes the endpoint's ingress serve `/pubsub/<topic>` as an SSE stream for it.
