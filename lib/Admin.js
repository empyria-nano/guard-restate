...
/**
 * Runs a raw SQL query against Restate's own introspection API (`POST {adminURL}/query`) — the
 * `sys_invocation`/`state`/`sys_promise`/... tables Restate exposes specifically so operators
 * and tooling can inspect what's running without an official client for it (see this file's own
 * "No official Admin API client" header note — same "fetch the REST endpoint directly" posture
 * as every other function here; this is simply the one REST endpoint none of them touch yet).
 * The endpoint takes a single JSON string, no parameter binding — build `sql` with any
 * caller-supplied values already escaped (single quotes doubled) before calling this.
 * @param {{restateAdminURL: string, sql: string}} params
 * @returns {Promise<Record<string, unknown>[]>}
 */
export async function queryRestate({ restateAdminURL, sql }) {
	const response = await fetch(`${restateAdminURL}/query`, {
		method: 'POST',
	...
	if (!response.ok) {
		...
		throw new restate.TerminalError(
			`Restate SQL query failed: ${response.status} ${response.statusText} — ${body.slice(0, 500)}`,
			{ errorCode: ERROR_INFRASTRUCTURE },
		)
	}
 ...
 * Restate's side (see the SDK's own workflow-submission semantics) — this is the
 * idiomatic "start if absent, otherwise leave it alone" primitive for a long-running,
 * once-per-key workflow, not just a plain fire-and-forget call.
 * @template P
 * @param {import('@restatedev/restate-sdk').Context} ctx Restate context of the CALLING
 *   handler — any handler type works here (plain service, virtual object, or another
 *   workflow), not just a workflow's own context. This only needs `ctx.genericSend`,
 *   which every Context exposes (declared on the base `Context` interface, not
 *   `WorkflowContext` — verified against the SDK's own type declarations); the earlier
 *   `WorkflowContext` annotation here reflected this repo's own usages so far
 *   (workflow-starts-sub-workflow), not an actual restriction.
 ...
 * @param {import('@restatedev/restate-sdk').Context} ctx Restate context of the CALLING
 *   handler — any handler type works here (plain service, virtual object, or another
 *   workflow); see {@link startSubWorkflow}'s own doc for why `Context`, not
 *   `WorkflowContext`, is the accurate type.
 ...
 * @property {(sql: string) => Promise<Record<string, unknown>[]>} query
...
export function createRestateAdmin({ restateAdminURL, restateURL }) {
	return {
		query: (sql) => queryRestate({ restateAdminURL, sql }),
